  ...
  if (history.length === 0) {
    return (
      <Card className="p-12 text-center shadow-soft">
        <p className="text-muted-foreground text-lg">{t("history.noHistory")}</p>
      </Card>
    );
  }
                ...
                <div className="flex items-center justify-between w-full pr-4">
                  <div className="text-left">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {summary.items.length} {summary.items.length !== 1 ? t("history.mealsPlural") : t("history.meals")}
                    </p>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {summary.totalInsulin.toFixed(1)}u {t("history.insulin")}
                    </div>
                  </div>
                </div>
              </AccordionTrigger>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-semibold text-foreground">{item.carbsEstimate}g {t("history.carbs")}</span>
                          <span className="text-sm text-muted-foreground">{formatTime(item.timestamp)}</span>
                        </div>
                        <div className="text-sm text-muted-foreground">
                          {item.insulinDose}u {t("history.insulinNeeded")}
                        </div>
                      </div>
                    </button>
                  ))}
